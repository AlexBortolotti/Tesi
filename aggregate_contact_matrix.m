...
% The Prem et al. estimates cut off at 80, so we sum all >75 year olds into one
...
agg_pop_pyramid=sum(sparse(aggregator,1:length(aggregator),pyramid),2)'; % sparse matrix defined here just splits pyramid into rows corresponding to coarse boundaries, then summing each row gives aggregated pyramid
...
%Test with recalculated aggregated matrix
% k_coarse = pop_weight_matrix * k_fine' * pop_no_weight';
k_coarse = k_coarse';
end
