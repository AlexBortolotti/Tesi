function [tspanned,result,tspan,refin] = model_HK(simulength, susc, cont_mat, agg_istat_pyr, tau, delta_E, prob_symp, gammaI, gammaA, initS, initE, initI, initA, initR, firstDay)
...
function dydt = odefun(t,y)
    ...
    dydt(1:6) = -susc.*((cont_mat.*y(1:6))*((y(2*6 + (1:6))+0.*y(3*6 + (1:6)))./agg_istat_pyr));
    %Latent dynamics
    dydt(6 + (1:6)) = susc.*((cont_mat.*y(1:6))*((y(2*6 + (1:6))+0.*y(3*6 + (1:6)))./agg_istat_pyr)) - delta_E*y(1*6+(1:6));
    %Infecteds dynamics
